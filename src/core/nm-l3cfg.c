                     ...
                     GArray               **conflicting)
{
        ...
        nm_l3_config_data_iter_obj_for_each (&iter, l3cd, &obj, NMP_OBJECT_TYPE_IP4_ADDRESS) {
            ...
            if (!addr_info) {
                ...
                if (addr_info->state <= NM_L3_ACD_ADDR_STATE_PROBING) {
                    ...
                    if (conflicting) {
                        if (!*conflicting)
                            *conflicting = g_array_new(FALSE,
                                                       FALSE,
                                                       nm_utils_addr_family_to_size(addr_family));
                        g_array_append_val(*conflicting, addr_info->addr);
                    }
                }
            }
    ...
    if (NM_IN_SET(addr_family, AF_UNSPEC, AF_INET6)
        ...
        nm_l3_config_data_iter_obj_for_each (&iter, l3cd, &obj, NMP_OBJECT_TYPE_IP6_ADDRESS) {
                ...
                if (conflicting) {
                    if (!*conflicting) {
                        *conflicting =
                        ...
                        g_array_append_val(
                            *conflicting,
                            NMP_OBJECT_CAST_IP6_ADDRESS(obj_state->os_plobj)->address);
                    }
                }
                continue;
            }
             ...
             * it's no longer tentative. This address is good. */
        }
        if (pending)
            return FALSE;
    }
